...
export default function DashShell({ children }) {
  ...
  const adminNav = [
    { to: '/admin', exact: true, label: 'Dashboard', icon: 'M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z' },
  ...
  const partnerNav = [
    { to: '/partner', exact: true, label: 'Dashboard', icon: 'M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z' },
  ...
  const customerNav = [
    { to: '/customer', exact: true, label: 'My certificates', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2' },
  ]
  ...
  const isActive = (item) => item.exact ? loc.pathname === item.to : loc.pathname.startsWith(item.to)
    ...
    <div className="dash-shell">
      <aside className="dash-sidebar">
        ...
        <div className="sb-section">
          ...
          {nav.map(item => (
            <Link key={item.to} to={item.to} className={`sb-item ${isActive(item) ? 'active' : ''}`}>
              <Icon d={item.icon} />
              {item.label}
            </Link>
          ))}
